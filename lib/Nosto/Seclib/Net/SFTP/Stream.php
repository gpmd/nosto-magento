...
/**
 * Nosto_Seclib_Net_SFTP Nosto_Seclib_Net_SFTP_Stream Wrapper
 *
 ...
 * @access  public
 */
class Nosto_Seclib_Net_SFTP_Stream
{
    /**
     * Nosto_Seclib_Net_SFTP instances
     *
    ...
    /**
     * Nosto_Seclib_Net_SFTP instance
     *
     * @var object
     * @access private
     */
    ...
    function _parse_path($path)
    {
        ...
        if ($host[0] == '$') {
            ...
            if (isset($sftp) && $sftp instanceof Nosto_Seclib_Net_SFTP) {
                $this->sftp = $sftp;
                return $path;
            }
            ...
            if (isset($context[$scheme]['privkey']) && $context[$scheme]['privkey'] instanceof Nosto_Seclib_Crypt_RSA) {
            ...
            if (isset(self::$instances[$host][$port][$user][(string) $pass])) {
                ...
                $this->sftp = new Nosto_Seclib_Net_SFTP($host, $port);
    ...
    function _stream_write($data)
    {
        ...
        $result = $this->sftp->put($this->path, $data, Nosto_Seclib_Net_SFTP::SOURCE_STRING, $this->pos);
        if (isset($this->notification) && is_callable($this->notification)) {
            if (!$result) {
                call_user_func($this->notification, STREAM_NOTIFY_FAILURE, STREAM_NOTIFY_SEVERITY_ERR, $this->sftp->getLastSFTPError(), NET_SFTP_OPEN, 0, 0);
                return 0;
     ...
     * If newname exists, it will be overwritten.  This is a departure from what Nosto_Seclib_Net_SFTP
     * does.
     *
     ...
     *
     * Also, nlist() is the best that this function is realistically going to be able to do. When an Nosto_Seclib_Net_SFTP client
     * sends a SSH_FXP_READDIR packet you don't generally get info on just one file but on multiple files. Quoting
     * the Nosto_Seclib_Net_SFTP specs:
     *
     ...
     * Flushes the output
     *
     * See <http://php.net/fflush>. Always returns true because Nosto_Seclib_Net_SFTP doesn't cache stuff before writing
     *
     * @return bool
     * @access public
     */
     ...
     *
     * Ignores the STREAM_URL_STAT_QUIET flag because the entirety of Nosto_Seclib_Net_SFTP_Stream is quiet by default
     ...
     * __call Magic Method
     *
     * When you're utilizing an Nosto_Seclib_Net_SFTP stream you're not calling the methods in this class directly - PHP is calling them for you.
