...
class Nosto_Seclib_Net_SSH1
{
    ...
    const CIPHER_IDEA = 1;
    /**
     * Nosto_Seclib_Crypt_DES in CBC mode
     */
    const CIPHER_DES = 2;
    /**
     ...
     *
     * Not supported nor is it defined in the official Nosto_Seclib_Net_SSH1 specs.  OpenSSH, however, does define it (see cipher.h),
     ...
     * Not supported.
     *
     * @internal According to the Nosto_Seclib_Net_SSH1 specs:
     *
     ...
     * Blowfish
     *
     * Not supported nor is it defined in the official Nosto_Seclib_Net_SSH1 specs.  OpenSSH, however, defines it (see cipher.h) and
    ...
    */
    ...
    function _connect()
    {
        ...
        $server_key_public_exponent = new Nosto_Seclib_Math_BigInteger($this->_string_shift($response[self::RESPONSE_DATA], ceil($temp['len'] / 8)), 256);
        ...
        $temp = unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $server_key_public_modulus = new Nosto_Seclib_Math_BigInteger($this->_string_shift($response[self::RESPONSE_DATA], ceil($temp['len'] / 8)), 256);
        ...
        $temp = unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_exponent = new Nosto_Seclib_Math_BigInteger($this->_string_shift($response[self::RESPONSE_DATA], ceil($temp['len'] / 8)), 256);
        ...
        $temp = unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_modulus = new Nosto_Seclib_Math_BigInteger($this->_string_shift($response[self::RESPONSE_DATA], ceil($temp['len'] / 8)), 256);
        ...
        $session_key = Nosto_Seclib_Crypt_Random::string(32);
        ...
        switch ($cipher) {
            ...
            case self::CIPHER_DES:
                $this->crypto = new Nosto_Seclib_Crypt_DES();
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
                $this->crypto->setKey(substr($session_key, 0,  8));
                break;
            case self::CIPHER_3DES:
                $this->crypto = new Nosto_Seclib_Crypt_TripleDES(Nosto_Seclib_Crypt_TripleDES::MODE_3CBC);
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
     ...
     *
     * An Nosto_Seclib_Net_SSH1 server will close the connection after a command has been executed on a non-interactive shell.  Nosto_Seclib_Net_SSH2
     * servers don't, however, this isn't an Nosto_Seclib_Net_SSH2 client.  The way this works, on the server, is by initiating a
     * shell with the -s option, as discussed in the following links:
     *
     * {@link http://www.faqs.org/docs/bashman/bashref_65.html http://www.faqs.org/docs/bashman/bashref_65.html}
     * {@link http://www.faqs.org/docs/bashman/bashref_62.html http://www.faqs.org/docs/bashman/bashref_62.html}
     *
     * To execute further commands, a new Nosto_Seclib_Net_SSH1 object will need to be created.
     *
    ...
    function _send_binary_packet($data)
    {
        ...
        $padding = Nosto_Seclib_Crypt_Random::string(8 - ($length & 7));
    ...
    function _rsa_crypt($m, $key)
    {
        ...
        */
        ...
        while (strlen($random) != $length) {
            $block = Nosto_Seclib_Crypt_Random::string($length - strlen($random));
            $block = str_replace("\x00", '', $block);
            $random.= $block;
        }
        $temp = chr(0) . chr(2) . $random . chr(0) . $m;

        $m = new Nosto_Seclib_Math_BigInteger($temp, 256);
    ...
    /**
     * Return a list of ciphers supported by Nosto_Seclib_Net_SSH1 server.
     *
     * Just because a cipher is supported by an Nosto_Seclib_Net_SSH1 server doesn't mean it's supported by this library. If $raw_output
    ...
    /**
     * Return a list of authentications supported by Nosto_Seclib_Net_SSH1 server.
     *
     * Just because a cipher is supported by an Nosto_Seclib_Net_SSH1 server doesn't mean it's supported by this library. If $raw_output
