 ...
 * DER blobs.
 *
 * Nosto_Seclib_File_ASN1 decodes and encodes DER formatted messages and places them in a semantic context.
 *
 * Uses the 1988 ASN.1 syntax.
 *
 * @category  File
 * @package   Nosto_Seclib_File_ASN1
...
class Nosto_Seclib_File_ASN1
{
    ...
    */
    ...
    {
        if ($encoded instanceof Nosto_Seclib_File_ASN1_Element) {
    ...
    function _decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                $current['content'] = new Nosto_Seclib_Math_BigInteger(substr($content, $content_pos), -256);
                break;
    ...
    function asn1map($decoded, $mapping, $special = array())
    {
        ...
        switch (true) {
            case $mapping['type'] == self::TYPE_ANY:
                ...
                if (isset($decoded['constant']) || !isset($this->ANYmap[$intype]) || (ord($this->encoded[$decoded['start']]) & 0x20)) {
                    return new Nosto_Seclib_File_ASN1_Element(substr($this->encoded, $decoded['start'], $decoded['length']));
                }
        ...
        switch ($decoded['type']) {
            ...
            case self::TYPE_ENUMERATED:
                ...
                if (isset($mapping['implicit'])) {
                    $temp = new Nosto_Seclib_Math_BigInteger($decoded['content'], -256);
                }
                if (isset($mapping['mapping'])) {
    ...
    {
        if ($source instanceof Nosto_Seclib_File_ASN1_Element) {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                if (!isset($mapping['mapping'])) {
                    if (is_numeric($source)) {
                        $source = new Nosto_Seclib_Math_BigInteger($source);
                    }
                    ...
                    $value = new Nosto_Seclib_Math_BigInteger($value);
            ...
            case self::TYPE_ANY:
                ...
                switch (true) {
                    ...
                    case is_int($source):
                    case $source instanceof Nosto_Seclib_Math_BigInteger:
